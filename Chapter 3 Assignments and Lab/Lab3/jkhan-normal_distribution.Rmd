...
###Answer
Both plots are nearly normal in that they're both unimodal, symmetric with a majority of the observations at the center of the plots. 

```{r}
hist(mdims$hgt, breaks=50, col='red')
```
```{r}
hist(mdims$hgt, breaks = 50, col='blue')
```
...
###Answer:
The points largely fall on the line except for a few data points on the higher end. Compared to the plot for the real data, the majority of the points fit either directly on the line or very close to it. 
...
###Answer
Yes. The plots for `fdims$hgt` and the simulations share the same main characteristics where the majority of the points lie directly on the line with only a few points on the end lie off the line. 


5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
###Answer    
Yes. The plots for `fdims$wgt` and the simulations share the same main characteristics where the majority of the points lie directly on the line with only a few points on the end lie off the line.  Therefore,`fdims$wgtt` follows a normal distribution.   
...
A. What is the probability that a randomly chosen woman is shorter than 150 cm?
...
B. What is the probability that a randomly chosen chosen woman weighs between 51 and 70 kg? 

```{r}
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)

lower_bound<-pnorm(q = 51, mean = fwgtmean, sd = fwgtsd)
upper_bound<-pnorm(q = 70, mean = fwgtmean, sd = fwgtsd)
...
```{r}
length(fdims$wgt[fdims$wgt >= 51 & fdims$wgt <= 70]) / length(fdims$wgt)
```

Both methods show that 68% of women are within one standard deviation(9.61 Kg) of the mean weight(60.60 Kg).  Further proof that the data is normally distributed.
...
The D plot for age contains discrete numerical variables. Age would be recorded as integers. It's a similar situation with the C plot, where the values are nearly discrete.
...
###Answer
The plot shows that the variable is right skewed as the data points move about the line at higher values.  The histogram confirms this finding. The histogram plot is unimodal where the highest values are below the mean. 
