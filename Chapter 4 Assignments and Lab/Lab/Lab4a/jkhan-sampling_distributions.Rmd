...
###ANSWER: 
The distribution of the diagram is unimodal with a right skew. 
...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    The distribution is unimodal with the highest frequency is in the middle.  It's normally distributed. 
...
The mean of samp2 is close to samp1 but they're not exactly the same. If we took more samples of larger sizes, we would get near the population sample but probably not the population mean. 
...
```
###ANSWER: 
There are 5000 elements in sample_means50. The sampling distribution is unimodal with the highest peak at the center. I do not expect the distribution to change with 50,000 sample means given the Central Limit Theory which states that samples, given indepence of the observations and size of each sample being larger than 30 follows a normal distribution.     
...
ANSWER: There are 100 elements in sample_means_small. Each element represents the sample mean for each sample size of 50 taken from the variable area. 
...
ANSWER: When the sample sizea get larger, the center stays nearly the same and the spread around it gets smaller. 
...
ANSWER: The point estimate for the population mean for price is 174932.3

```{r}
samp11 <- sample(price, 50)
```
...
###Answer
The shape of the sampling distribution is unimodal and symmetrical with highest mean sample price in the middle. Based on the sampling distribution, I estimate that the mean house price is somewhere near 180,000. The actual mean price is 180796.1
...
```
###Answer
The shape of the sampling distribution is unimodal and symmetrical with highest mean sample price shaded to the right of 180000. There's less variability in the distribution when compared to sample sizes of 50. Based on the sampling distribution, I estimate that the mean house price is slightly above 180,000. 
...
###Answer
Sampling distribution in 3 has a smaller spread than the Sampling distribution in 2. We can conclude that we would prefer a distribution with a smaller spread (larger sample size) in order to estimate the true value.
