...
The distribution of the sample is unimodal, with a slight right skew.  However, you could reasonably state that it's normally distributed. The typical size is between 1000 and 1500 square feet or 1250 square feet. I interpreted that way since the range of 1000 to 1500 has the higest number of observations.
...
No.  Selection of each sample is random. Any other sample selected would have different observations in it. 
...
2. The sample size is larger than 30.
...
###ANSWER
The 95% confidence interval provides a plausible range of values for estimating the true population mean based on the sample mean plus/minus 1.96 times the standard error. 
...
###ANSWER
Yes.  The confidence interval was 1297.525, 1528.109, and the true average is within in this interval. 
...
Cannot answer since this is an online course, and there are no neighbors.
...
48 out of the 50 Confidence Intervals (96%) contained the true population mean.  Although not exactly, at 96% this equals the 95% confidence interval. 
...
###ANSWER
At a 90% confidence interval, the critical value is +/- 1.64. 
...
At 90% confidence intervals, I found 5 intervals out of the 50 that did not contain the true population mean. 45/50 equates to 90% which is exactly like the confidence interval.    
