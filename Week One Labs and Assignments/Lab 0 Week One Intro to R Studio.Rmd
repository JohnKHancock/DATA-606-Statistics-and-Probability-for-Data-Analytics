---
title: "Lab 0  Week One: Introduction to R and RStudio"
...
```
```{r}
setwd('C:/Users/jkhan/Documents/CUNY/Fall 2018/DATA606/Week One due 09-02-2018/Lab 0')
```
...
```{r}
print('The highlights of the plot are (1) starting in 1640 to 1650, there was a steep decline in the baptismal of girls. (2) Then starting in 1659 or so, there was an exponential growth until 1700 where there was another v-shape decline and growth')
```
...
```{r}
print('The years -- a numerical, discrete variable -- are from 1940 to 2002')
present$year
```
```{r}
print('The dimensions of the data frame are:')
dim(present)
print('There are 63 observations for three independent variables')
...
```{r}
print('There are 19 less observations for the present data frame (63) than the arbuthnot data frame (82), but there are the same number of independent variables for each.')
```
...
```{r}
print('The proportion of boys born to girls peaked twice in the 1940s with a steady decline from the last peak prior to 1950. The proporation started a steep decline. and have never returned to their peek.')
```
...
```{r}
print('First, create a new column in the data frame for total births')
```

```{r}
print('The new data frame now has an additional calculated column called, total_births.')
```
...
```
```{r}
print('Finally, subset the present data frame by using a conditional search that finds the year with the maximum number of births which is 1961.') 
