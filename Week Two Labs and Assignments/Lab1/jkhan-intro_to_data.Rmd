...
###Answer:
By running the dim function, there are 20,000 cases and 9 variables in this dataset, and by running the sapply function, we can see that the "genhlth" and gender variables are categorical. Variables exerany, hlthplan, and smoke100 are numeric discrete. Variables height, weight, wtdesire, and age are numeric/continuous.     
...
###Answers
There are 9,569 males in the survery, and 23% of the respondents report excellent health.
...
###Answer
More men (5,022) have smoke 100 cigarettes than have women (4,419)
...
###Answers
The boxplots show that the median BMI value is consistent with how people generally report on the condition of their health.  However, there are a lot of outliers in the data. 

The boxplots below show the relationship between BMI and exerany. The plots show that those who have exercised in the last month have a lower median BMI.   
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    ANSWER: "Wdiff" is a discrete, numeric data.  If it is 0, then that means that the person's actual weight matches their desried weight.  If it's positive, then that means the person feels that they need to add more weight.  If negative, then the person feels that they need to lose that amount of weight.
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    ANSWER: The distribution is unimodal with the highest frequency of observations being 0.  This means that most of the respondents feel that they are currently at their desired weight.
    
```{r}
hist(wdiff, breaks = seq(-600,600, l=100))
```
...
ANSWER: By looking at weight desired by gender, men have a higher median desired weight than do women. Thus men and women view their weight differently.      
...
ANSWER: The mean and standard deviation for weight are 169.68 and 40.08 respectively.  The poportion of the observations that are within one standard deviation of the mean weight is .07076 or 71%. 
...
```{r}
one_sd_above <- mean_weight + stdev 
one_sd_below <- mean_weight-stdev 

within_one_stdev <- length(cdc$weight[cdc$weight >= one_sd_below & cdc$weight <= one_sd_above ])
proportion <- within_one_stdev / length(cdc$weight)
proportion
